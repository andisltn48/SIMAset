    ...
    <ul class="mt-5">
        ...
        @if (session('role') == 'Peminjam')
            ...
            <li class="list">
                <b></b>
                <b></b>
                <a href="{{route('peminjaman.list-peminjaman')}}">
...
<script>
    let savedLink = localStorage.getItem("linkWindow");
    let current_window = window.location.href;
    if (current_window != savedLink) {
        localStorage.removeItem('indexNum');
    }
    let list = document.querySelectorAll('.list');
    console.log(list.length)
    if (localStorage.getItem("indexNum") > 0) {
        let index = localStorage.getItem("indexNum");
    ...
    for (let i = 0; i < list.length; i++) {
        list[i].onclick = function() {
            ...
            localStorage.setItem("indexNum", i);
            localStorage.setItem("linkWindow", current_window);
        }
    }
