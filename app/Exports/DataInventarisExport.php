...
class DataInventarisExport implements FromQuery, WithHeadings, WithColumnFormatting
{
    use Exportable;
    ...
    */
    ...
    {
        $data_inventaris = DataInventaris::query()->select([
            'nama_barang',
            'kode',
            'nup',
            'uraian_barang',
        ...
        if ($this->unit != null) {
            $data_inventaris->where('unit', $this->unit);
        }
        if ($this->kondisi != null) {
            $data_inventaris->where('kondisi', $this->kondisi);
        }
        if ($this->koderuangan != null) {
            $data_inventaris->where('kode_ruangan', $this->koderuangan);
        }
        if ($this->ruangan != null) {
            $data_inventaris->where('tahun_pengadaan', $this->tahunpengadaan);
        }
        if ($this->kodebarang != null) {
            $data_inventaris->where('kode', $this->kodebarang);
        }
        if ($this->nup != null) {
            $data_inventaris->where('nup', $this->nup);
        }
