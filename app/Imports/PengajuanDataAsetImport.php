        ...
        if ($row["jumlah"] > 1) {
            ...
            for ($current_nup; $current_nup <= $last_nup ; $current_nup++) {
                ...
                if ($test_nup->count()  == 0 AND $test_nup2->count()  == 0) {
                    ...
                    $success = DetailLogImportPengajuan::create([
                        'row' => $row["no"],
                    ...
                    $failed = DetailLogImportPengajuan::create([
                        ...
                        'nama' => $row["nama_barang"],
                        'status' => 'Failed',
                        'message' => 'Barang dengan nup : '.$current_nup.' dan kode barang : '.$row["kode_barang"].' sudah terdaftar atau telah diajukan',
                        'import_id' => $this->id_import
                    ]);
                }
            ...
            $test_nup2 = DataPengajuan::where('kode',$row["kode_barang"])->where('nup', $row["nup_awal"])->get();
                ...
                if ($test_nup->count()  == 0 AND $test_nup2->count()  == 0) {
                    ...
                    $failed = DetailLogImportPengajuan::create([
                        ...
                        'nama' => $row["nama_barang"],
                        'status' => 'Failed',
                        'message' => 'Barang dengan nup : '.$row["nup_awal"].' dan kode barang : '.$row["kode_barang"].' sudah terdaftar atau telah diajukan',
                        'import_id' => $this->id_import
                    ]);
                }
        }
